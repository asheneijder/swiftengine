...
class MxPacs008Parser implements SwiftMessageParser
{
    public function parse(string $content): array
    {
        ...
        // Structure usually: [SAA Wrapper] -> Body -> AppHdr + Document -> FIToFICstmrCdtTrf
        ...
        // Note: pacs.008 can be bulk. This parser takes the FIRST transaction for the CSV row.
        // If bulk processing is needed, the Service needs to support returning multiple rows.
        ...
        if ($tx) {
            ...
            // -- Payment Details --
            $data['Pmt_Method']     = SwiftMxParserUtil::get($tx->PmtTpInf->LclInstrm->Prtry); // e.g. BKT
            $data['Svc_Level']      = SwiftMxParserUtil::get($tx->PmtTpInf->SvcLvl->Cd);       // e.g. SDVA
            $data['Category_Purp']  = SwiftMxParserUtil::get($tx->PmtTpInf->CtgyPurp->Cd);     // e.g. CORT
            ...
            $data['Charge_Bearer']  = SwiftMxParserUtil::get($tx->ChrgBr); // DEBT, CRED, SHAR
        ...
        // --- 4. Flatten for CSV output (Optional: Add header info to row) ---
        // (Data array above is already flat)

        // --- 5. Return Metadata for Service ---
        // Determine Header Sender/Receiver if available
        ...
        return [
            ...
            // Merge all data fields for the CSV row
            ...$data 
        ];
    }
